...
2. Wrap heavy operations with `globalStorage.getOrCall()` to compute value once and re-use in all tests:
    ```ts
    const value = await globalStorage.getOrCall('some-key', async () => {
...
```ts
const value = await globalStorage.getOrCall(`some-key-${id}`, async () => {
...
test.use({ 
    storageState: async ({ context }, use) => {
        const storageState = await globalStorage.getOrCall('auth', async () => {
...
test.use({ 
    storageState: async ({ context }, use) => {
        const storageState = await globalStorage.getOrCall(`auth-${username}`, async () => {
            const loginPage = await context.newPage();
            await loginPage.goto('https://example.com');
...
test.before(async () => {
  userId = await globalStorage.getOrCall('userId', async () => {
...
test('test', async ({ page }) => {
  await page.route('/api/cats/**', (route) => {
    const json = globalStorage.getOrCall('cats-response', async () => {
...
test('test', async ({ page }) => {
  await page.route('/api/cats/**', (route, req) => {
    ...
    const json = globalStorage.getOrCall(storageKey, async () => {
       const response = await route.fetch();
       return response.json();
    });
